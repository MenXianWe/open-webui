...
In QLCodeChat this surfaces as:
...
class AuthTokenMiddleware:
    ...
    This is useful when QLCodeChat sits behind a reverse proxy that
    consumes the ``Authorization`` header for its own authentication —
    set the env var to a unique header (e.g. ``X-QLCodeChat-Key``) so
