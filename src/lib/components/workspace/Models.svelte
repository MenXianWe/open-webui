<script lang="ts">
	...
	import { WEBUI_API_BASE_URL, WEBUI_BASE_URL, WEBUI_BRAND_LOGO_URL } from '$lib/constants';
	import {
		createNewModel,
		deleteModelById,
	...
	const shareModelHandler = async (model) => {
		toast.success($i18n.t('Redirecting you to QLCodeChat Community'));

		const url = 'https://qlcodeapi.com';
									...
									<div class="flex bg-white rounded-2xl">
										...
										>
											<img
												...
												decoding="async"
												on:error={(e) => {
													e.target.src = WEBUI_BRAND_LOGO_URL;
												}}
											/>
										</div>
									</div>
	...
	{#if $config?.features.enable_community_sharing}
		<div class=" my-16">
			<div class=" text-xl font-medium mb-1 line-clamp-1">
				{$i18n.t('Made by QLCodeChat Community')}
			</div>

			<a
				class=" flex cursor-pointer items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-850 w-full mb-2 px-3.5 py-1.5 rounded-xl transition"
				href="https://qlcodeapi.com/models"
				target="_blank"
			>
